...
26.3の開発バージョンは `26.3-snapshot-1`〜`26.3-snapshot-10`と`26.3-pre-1`〜`26.3-pre-3`、`26.3-rc-1`、`26.3-rc-2`を完全一致で選びます。各開発バージョンでdata pack formatと破壊的変更が進むため、「26.3向け」や「最新snapshot向け」という曖昧な対象では生成しません。
